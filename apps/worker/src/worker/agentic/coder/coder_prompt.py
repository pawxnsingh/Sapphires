...
TOOL_USAGE_INFO = """
<file_operations>
  You have access to file system tools to create and modify files. YOU MUST USE THESE TOOLS to write code.

  CRITICAL: DO NOT output code in <sapphiresArtifact> or <sapphiresAction> tags. Instead, USE THE TOOLS:

  <available_tools>
    - write_file: Create or overwrite a file with content
      - path: relative path from project root (e.g., "app/(tabs)/index.tsx")
  ...
  <file_writing_rules>
    1. ALWAYS use write_file tool to create or update files - never just output code in text
    2. Use read_file FIRST to understand existing code before modifying
    3. Provide COMPLETE file content - no placeholders or "rest remains the same"
    4. Use relative paths from project root (e.g., "components/TodoItem.tsx")
    5. Create parent directories automatically (write_file handles this)
    6. You can call multiple write_file tools in parallel for independent files
  </file_writing_rules>
